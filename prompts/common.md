...
正文里出现的每个概念先用自然语言交代。「连接看门狗发出的取消通知」读得懂，「cancel token 在 select 分支里被 poll」得先去读源码才知道在说什么。要提某个语言特性或 API 行为时，描述它的性质，符号名放进括号作补充。
...
**怎么发生的** —— 客户端要发一条请求，得先把它排进发送队列。对端停止接收、
队列槽位占满时，这条请求就停在那里等。

它等待期间听不见「收工」信号：取消通知只在每次准备取下一条待发消息之前被检查
一次 [1]，一旦卡进发送动作内部就再也轮不到检查 [2]。文档说这时通道会关闭、于是
报错返回，而通道的所有权握在发送循环手里，那个函数不返回，通道就不会关。

外部也救不了 —— 唯一能强行掐断的手段要求独占整个客户端，调用方此刻正借用着它。

**引用**

[1] task.rs:480 —— 取消信号唯一被检查的地方
[2] task.rs:491 —— 卡住的发送调用
[3] client.rs:254 —— 对外的请求入口
```
