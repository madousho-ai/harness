...
| `voice.md` | 语气、讲代码的方式、什么时候画图、语言规则 |
...
**plan-arch** 定架构。读 `spec.md` 与代码库现状，逐项收敛模块划分、模块职责、公开接口、模块间互动、数据流、业务流程六个问题，写进同目录的 `arch.md`。精度停在大体运作逻辑，只有用户点名担心的地方才下钻到实现细节，落进 `## 下钻`。数据流与业务流程这两项的产出形态是参与方交代 + 一张图 + 一段走查 —— 一个节点等于一次跨模块调用、一次落库或一个决定分岔，函数内部的语句留给实现阶段。画法在第一张图之前问用户要 mermaid 还是 ASCII，一份文档统一一种。
...
命令不预检这两道闸。第一次真派人就是检验 —— 闸关着的话 `task` 调用当场报错，命令终止，错误原文给用户，不会悄悄退回两层假装在跑。
