...
一个上下文扛得动十三四个任务。在这条线以内的 phase 整个交给一个实现者；超过就把**实现**对半切 —— 二十个切成十加十，二十七个切成十三加十四 —— 对半之后仍然超线的才切三段。一段一个全新的实现者，按顺序跑。

切口从中点起找，落到最近的那道分组边界上：跨几个 crate 的 phase 通常拿小标题把它们分好了，那些标题就是切口。落点要验，不能假定。任务清单本来就是依赖序写的，它声明的每条顺序约束都该跨过切口指向后面；有一条指回来，说明两半反了。真正承重的判据是绿不绿 —— 第一段的测试要在第二段一行都没落地的情况下全过。切出一个测试等着后面任务才能绿的段，这一段收尾时套件是红的，下一段接手的仓库跟坏掉的仓库长得一模一样。
